...
4. Guides, weapons, bosses, combat topics and characters start as draft. Use stable lowercase hyphenated slugs and real editorial dates. `sources`, `lastVerified` and evidence status are optional so fake values are never needed. Review facts and source support before marking published. Do not use model memory as evidence. Avoid H1 in Markdown because the shared detail layout supplies it.
...
6. Run `npm ci`, `npm run build`, `npm run test:build-hygiene` and `git diff --check`. Production builds must remove prior `dist/` output first, so it contains only the current publishable source projection; inspect that deleted, withdrawn or draft content leaves no HTML page or sitemap URL. For route/redirect changes, verify actual HTTP status and chain in the relevant hosting environment. If unavailable, explicitly record unverified status.
7. Test publication exclusion with temporary generic fixtures when changing the publication pipeline; delete or withdraw fixtures and rebuild clean before handoff. Confirm every content area removes its HTML page and sitemap URL. Do not publish fake game content for tests.
